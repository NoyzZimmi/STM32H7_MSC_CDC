...
/* Private variables ---------------------------------------------------------*/
/* USER CODE BEGIN PV */
extern TX_EVENT_FLAGS_GROUP UsbxMscEventFlag;
                       ...
                       ULONG number_blocks, ULONG lba, ULONG *media_status)
{
  ...
  if (GPIO_SD_isCardPresent())
  {
      ...
      /* Start the Dma write */
      if (HAL_SD_ReadBlocks_IT(&hsd1, data_pointer, lba, number_blocks) != HAL_OK)
      {
          Error_Handler();
      }

      /* Wait on readflag until SD card is ready to use for new operation */
      if (tx_event_flags_get(&UsbxMscEventFlag, SD_READ_FLAG, TX_OR_CLEAR, &ReadFlags, TX_WAIT_FOREVER) != TX_SUCCESS)
      {
          Error_Handler();
      }
  }
                        ...
                        ULONG number_blocks, ULONG lba, ULONG *media_status)
{
  ...
  if (GPIO_SD_isCardPresent())
  {
      ...
      /* Start the Dma write */
      if(HAL_SD_WriteBlocks_IT(&hsd1, data_pointer, lba, number_blocks) != HAL_OK)
      {
          Error_Handler();
      }

      /* Wait on writeflag until SD card is ready to use for new operation */
      if (tx_event_flags_get(&UsbxMscEventFlag, SD_WRITE_FLAG, TX_OR_CLEAR,
...
{
  if (tx_event_flags_set(&UsbxMscEventFlag, SD_WRITE_FLAG, TX_OR) != TX_SUCCESS)
  {
    Error_Handler();
  }
}
...
{
  if (tx_event_flags_set(&UsbxMscEventFlag, SD_READ_FLAG, TX_OR) != TX_SUCCESS)
  {
    Error_Handler();
  }
}
